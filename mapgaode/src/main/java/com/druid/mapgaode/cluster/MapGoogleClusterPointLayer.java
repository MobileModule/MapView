...
import com.druid.mapgaode.bean.GoogleClusterMarkerBean;
import com.druid.mapgaode.bean.GoogleClusterMarkerItem;
...
public class MapGoogleClusterPointLayer extends ClusterPointLayer<Marker> implements LayerApi<MapView, AMap>,
...
//        zoomFirstCamera(false);
    ...
    private void createClusterMarkerLayer() {
        if (googleClusterManager == null) {
            googleClusterManager = new GoogleClusterManager<GoogleClusterMarkerBean>(context, aMap);
            googleClusterManager.setClusterMarkerClickListener(this::mapMarkerClick);
            MapConstantUtils.configClusterStyle(context, googleClusterManager, aMap);
                ...
                public boolean onClusterClick(Cluster<GoogleClusterMarkerBean> cluster) {
                    ...
                    if (cluster.getSize() > 0) {
                        if (aMap.getCameraPosition().zoom < aMap.getMaxZoomLevel()) {
                            float zoom = aMap.getCameraPosition().zoom + 2f;
                            if (aMap.getCameraPosition().zoom < aMap.getMaxZoomLevel()) {
            ...
            new ClusterManager.OnClusterItemClickListener<GoogleClusterMarkerBean>() {
                ...
                public boolean onClusterItemClick(GoogleClusterMarkerBean item) {
                    ...
                    if (index < sources.size()) {
                        ...
                        aMap.animateCamera(CameraUpdateFactory.newLatLng(item.getPosition()), clusterMarkerToLargeCallback);
                    }
                    return true;
                }
            };

    AMap.CancelableCallback clusterMarkerToLargeCallback = new AMap.CancelableCallback() {
        @Override
    ...
    private void addClusterMarkerLayer() {
        ...
        mapClear();
        aMap.setOnMarkerClickListener(googleClusterManager);
    ...
    AMap.CancelableCallback markerToLargeCallback = new AMap.CancelableCallback() {
        @Override
        public void onCancel() {
    ...
    protected void cameraToClusterMarkerBounds() {
        if (clusterLayerLatLngBounds != null) {
            aMap.animateCamera(CameraUpdateFactory.newLatLngBounds(clusterLayerLatLngBounds, MapSetting.MAP_BOUNDS_PADDING));
        } else {
            if (sources.size() == 1) {
                ...
                if (MapUtils.validLatLng(lat, lng)) {
                    aMap.animateCamera(CameraUpdateFactory.newLatLngZoom(new LatLng(lat, lng), 9f));
                }
            }
        }
    }
...
//                            , markerToLargeCallback);
                    aMap.moveCamera(CameraUpdateFactory.newLatLng(LatLngUtils.mapLatLngFrom(marker.latLng)));
                    unClusterMarkerLarge();
                    return true;
                } else {
                    return false;
            ...
            if (!TextUtils.isEmpty(title)) {
                try {
                    ...
                    if (index != -1) {
                        nowMarkerClickIndex = index;
                        ClusterMarkerBean clusterMarker = sources.get(index);
                        aMap.animateCamera(CameraUpdateFactory.newLatLng(marker.getPosition()), markerToLargeCallback);
                    }
                } catch (Exception ex) {
