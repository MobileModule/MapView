...
import com.druid.mapgaode.utils.LatLngEvaluator;
...
public abstract class GaodeLineLayer extends LineDrawLayer implements MapCameraIdleListener {
    public static final String TAG = GaodeLineLayer.class.getName();
    protected MapView mapView = null;
    protected AMap aMap = null;
    protected DruidMapView druidMapView=null;

    public GaodeLineLayer(Context context) {
        super(context);
    }
...
//            ArrayList<LatLngBean> points = AverageLineUtils.getArrowLine(source);
            ...
            for (int i = 0; i < graphicPoints.size() - 1; i++) {
                ...
                LatLng latlng2 = aMap.getProjection().fromScreenLocation(point_last);
                ...
                if (length > 80f) {
                    ...
                    LatLng center = aMap.getProjection().fromScreenLocation(point_center);
            ...
            for (int i = 0; i < points.size() - 1; i++) {
                if (i % 2 == 0) {
                    ...
                    MarkerOptions markerOption = new MarkerOptions().icon(descriptor)
                            .position(point_start)
                            .title("")
                            .rotateAngle(Float.parseFloat(String.valueOf(rotate)))
                            .anchor(0.5f, 0.5f)
                            .draggable(false)
    ...
    protected void drawPlayPoint(LatLng latLng) {
        if (preLatLng != null) {
            ...
            if(playMarker!=null){
                ...
                playMarker.setRotateAngle(bearing);
            }else {
                ...
                MarkerOptions markerOption = new MarkerOptions().icon(descriptor)
                        ...
                        .draggable(false)
                        .zIndex(MapConstantUtils.ZINDEX_MARKER);
                playMarker = aMap.addMarker(markerOption);
            }
        }
    }
    ...
    private float getBearing(LatLng from, LatLng to) {
        ...
        bear -= aMap.getCameraPosition().bearing;
        return bear;
    }
    //endregion
