...
public abstract class DrawFencePolygon extends FenceDrawPolygonLayer<Marker> {
    ...
    public boolean completeDrawFence(boolean needMoveCamera) {
        ...
        fencePolygon = googleMap.addPolygon(polygonOptions);
        //
    ...
    private void cameraMoveToFencePoint(LatLng latLng) {
        CameraPosition cameraPosition = new CameraPosition.Builder()
                ...
                .zoom(googleMap.getCameraPosition().zoom)
                .build();
        googleMap.animateCamera(CameraUpdateFactory.newCameraPosition(cameraPosition), 200,
    ...
    protected boolean completeEditorFencePoint() {
        if (editorFencePointIndex != -1) {
            fencePoints.addFencePoint(editorFencePointIndex, LatLngUtils.mapLatLngParse(googleMap.getCameraPosition().target));
            this.editorFencePointIndex = -1;
            this.tempClickPoint = null;
    ...
    protected void cameraToFenceBounds() {
        if (fenceLayerLatLngBounds != null) {
            googleMap.animateCamera(CameraUpdateFactory.newLatLngBounds(fenceLayerLatLngBounds, MapSettingConstantUtils.MAP_BOUNDS_PADDING));
        }
    }
